...
export function tabs(a: TabsArgs = {}): string {
  ...
  // daisyUI's structure classes (tabs, tab, tab-content) stay; ita-tabs adds the .italia look.
  const list = cx(
    "tabs",
    variant === "card" ? "tabs-lift ita-tabs ita-tabs-card" : "tabs-border ita-tabs",
    a.fullWidth && "ita-tabs-fill",
    a.iconAbove && "ita-tabs-stacked",
  );
  const body = items
    .map((it) => {
  ...
  <div class="tab-content">${it.content ?? ""}</div>`;
    })
    .join("\n");
...
export function tabsNav(a: TabsArgs = {}): string {
  ...
  const list = cx("tabs tabs-border ita-tabs ita-tabs-nav", a.fullWidth && "ita-tabs-fill");
  const links = items.map(
    (it) =>
      ...
      }>${it.icon ? icon(it.icon, "") : ""}<span>${it.label}</span></a>`,
  );
...
export const doc: ComponentDoc = {
  ...
  classes: ["ita-tabs", "ita-tabs-card", "ita-tabs-fill", "ita-tabs-stacked", "ita-tabs-nav"],
...
export function Tabs({ items }: { items: { label: string; content: ReactNode }[] }) {
  ...
  return (
    <div>
      <div role="tablist" className="tabs tabs-border ita-tabs"
